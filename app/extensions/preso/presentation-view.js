...
export class PresentationView {
  ...
  async renderSlide(slide) {
    ...
    this.floatingEl.className = "slide-preview is-visible"; // Reset classes
    if (this.currentTheme) {
      this.floatingEl.classList.add(this.currentTheme);
    }
    ...
    if (footnoteDefs.size > 0) {
      ...
      if (footnoteRefMap.size > 0) {
        ...
        const footnotesContainer = this.floatingEl.appendChild(
          document.createElement("div"),
        );
        footnotesContainer.className = "slide-footnotes-container";
        const footnotesList = footnotesContainer.appendChild(
        ...
        for (const [id] of sortedRefs) {
          ...
          if (content) {
            const listItem = footnotesList.appendChild(
    ...
    if (bgImageInfos.length > 0) {
      ...
      const sliceContainer = this.floatingEl.appendChild(
        document.createElement("div"),
      );
      ...
      const bgWrapper = this.floatingEl.appendChild(
        document.createElement("div"),
      );
      ...
      const imagePane = this.floatingEl.appendChild(
        document.createElement("div"),
      );
      imagePane.className = "split-image-pane";
      const textPane = this.floatingEl.appendChild(
        document.createElement("div"),
      );
  ...
  async setExtras(options) {
    ...
    if (hasHeader) {
      const headerContainer = this.floatingEl.appendChild(
        document.createElement("div"),
      );
      headerContainer.className = "slide-header-extras-container";
      const leftGroup = headerContainer.appendChild(
        document.createElement("div"),
      );
      leftGroup.className = "extras-left-group";
      const rightGroup = headerContainer.appendChild(
        document.createElement("div"),
      );
      rightGroup.className = "extras-right-group";

      if (options.topLeftIconSrc) {
        const img = leftGroup.appendChild(document.createElement("img"));
        img.src = options.topLeftIconSrc;
        img.className = "top-left-icon";
      }
      if (options.headerText || options.headerImageSrc) {
        const headerContent = leftGroup.appendChild(
          document.createElement("div"),
        );
        headerContent.className = "header-content";
        if (options.headerImageSrc) {
          const img = headerContent.appendChild(document.createElement("img"));
          img.src = options.headerImageSrc;
          img.className = "header-image";
        }
        if (options.headerText) {
          const headerTextEl = headerContent.appendChild(
            document.createElement("span"),
          );
          headerTextEl.innerHTML = marked.parse(options.headerText);
          this.applyInlineCssDirectives(headerTextEl);
        }
      }
      if (options.topRightIconSrc) {
        const img = rightGroup.appendChild(document.createElement("img"));
        img.src = options.topRightIconSrc;
        img.className = "top-right-icon";
      }
    }

    if (hasFooter) {
      const footerContainer = this.floatingEl.appendChild(
        document.createElement("div"),
      );
      footerContainer.className = "slide-footer-extras-container";
      const leftGroup = footerContainer.appendChild(
        document.createElement("div"),
      );
      leftGroup.className = "extras-left-group";
      const rightGroup = footerContainer.appendChild(
        document.createElement("div"),
      );
      rightGroup.className = "extras-right-group";

      if (options.footerText || options.footerImageSrc) {
        const footerContent = leftGroup.appendChild(
          document.createElement("div"),
        );
        footerContent.className = "footer-content";
        if (options.footerImageSrc) {
          const img = footerContent.appendChild(document.createElement("img"));
          img.src = options.footerImageSrc;
          img.className = "footer-image";
        }
        if (options.footerText) {
          const footerTextEl = footerContent.appendChild(
            document.createElement("span"),
          );
          footerTextEl.innerHTML = marked.parse(options.footerText);
          this.applyInlineCssDirectives(footerTextEl);
        }
      }
      if (options.slideNumber) {
        const slideNumEl = rightGroup.appendChild(
          document.createElement("div"),
        );
        slideNumEl.className = "slide-number";
        slideNumEl.textContent = options.slideNumber;
      }
    }
  }
